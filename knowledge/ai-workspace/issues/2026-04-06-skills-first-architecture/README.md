...
实际使用中，模型在记录"确保不再犯错"的规则时，倾向于堆积到 `AGENTS.md`，而非写入对应方向的 skill 或 rule。

这导致 `AGENTS.md` 逐渐膨胀，偏离"导航文件"的定位。
...
- `git-workflow`：由 `git.mdc` 演进而来，承接工作区提交、项目提交、分组策略和 Conventional Commits 规范
...
- `.cursor/rules/*.mdc` 仅作为剩余语言类规范的过渡入口，已有工程流程优先迁入 `.agents/skills/`
- addyosmani/agent-skills 的理念可借鉴（anti-rationalization、verification gate、process-not-prose），但单个 skill 控制在 100-200 行，不照搬

## 0x03 参考

- [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills)：工程类 skills 参考，覆盖 define → ship 生命周期
