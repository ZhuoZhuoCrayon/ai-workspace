# bkmonitor 知识库

> 蓝鲸监控平台 (bk-monitor/bkmonitor)

## 索引

### Issues

| 需求 | 标签 | 状态 | 更新日期 |
|------|------|------|----------|
| [告警日志查询支持 Doris 数据源](./issues/2026-03-12-log-query-doris-support/README.md) | `log` `unify-query` `doris` `data-source` | 待实现 | 2026-03-12 |
| [APM 支持应用级别配置](./issues/2026-03-04-apm-app-level-config/README.md) | `apm` `service-config` `log-relation` `code-redefine` `app-level` | 设计中 | 2026-03-08 |
| [日志 UnifyQuery 环境变量白名单与 query_string 增强](./issues/2026-03-05-log-uq-env-whitelist-and-query-string/README.md) | `log` `unify-query` `data-source` `query-string` `config` | 已实现 | 2026-03-05 |
| [APM 支持跨应用共享数据源](./issues/2026-03-03-apm-shared-datasource/README.md) | `apm` `datasource` `es` `shared-storage` | 设计中 | 2026-03-03 |
| [日志数据源切换 unify-query](./issues/2026-02-10-log-ds-to-unify-query/README.md) | `log` `unify-query` `data-source` | 对账中 | 2026-02-10 |

### Troubleshooting

| 分类 | 路径 | 数量 | 说明 |
|------|------|------|------|
| 排障经验 | [troubleshooting/](./troubleshooting/INDEX.md) | 1 篇 | 问题排查与修复记录 |

### Snippets

代码片段集合，包含告警、APM、自定义上报等常用操作。

- [Snippets 索引](./snippets/INDEX.md) — 16 个代码片段
