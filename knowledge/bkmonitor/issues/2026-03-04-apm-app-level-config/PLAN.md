...
|:--|:--|:--|:--|:--|:--|:--|
| 服务配置 DB 表变更 | PR-1 | 本周发布后优先合入 | 低 | 1）可能影响序列化器；2）已检查旧引用基本废弃，整体影响可控 | 序列化器相关接口响应与字段结构是否正常 | 待开始 |
| 关系查询与同步逻辑收口 | PR-2 | PR-1 稳定后合入 | 中 | 1）模型新增收口方法本身影响低；2）迁移并废弃旧方法，调用面较广 | 所有调用收口逻辑的接口回归；同步结果需与旧逻辑一致 | 待开始 |
| 日志关联、返回码重定义与返回码备注改造 | PR-3 | PR-2 稳定后合入 | 中 | 1）接口协议变更；2）返回码备注新增应用级合并语义；3）配置功能行为变更 | 配置层接口功能完整测试；查询层验证规则与备注优先级是否生效 | 待开始 |
...
| PR-3 | 协议变更联调 + 配置生效验证，重点覆盖返回码备注的全局/服务合并优先级 |

## 0x04 实施进展

| 时间 | 对应设计片段 | 结论调整概要 | 改动 / 验证 |
|:--|:--|:--|:--|
| 2026-04-09 | `0x01.a` `0x01.c` `0x01.e` `0x02.c` | 确认返回码备注不进入关系表，改为统一落在应用级 `ApmMetaConfig.code_remarks`；保留 `kind` 作为备注维度；服务视角 `set` 以 `kind + code + remark` 合并 `service_names`，并强制移除同 `(kind, code)` 下的旧冲突记录；服务视角 `get` 仅做后置补齐内置默认备注，全局视角不返回内置项。 | 已核对 `GetCodeRemarksResource` / `SetCodeRemarkResource`、`ApmMetaConfig`、`CodeRedefinedConfigRelation`、现有前端调用面和 tRPC 内置错误码来源，已据此更新方案主干与默认备注表。 |
...
*制定日期：2026-03-04 ｜ 更新日期：2026-04-09*
