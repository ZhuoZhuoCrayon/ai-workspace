...
| 时间 | 结论性进展 |
| --- | --- |
| `2026-06-03 01:00` | [a] 回归源码后确认节点维度应映射为事件字段 `host`。<br />[b] 主机目标前置查询 K8S 节点，未命中返回 `None`。<br />[c] K8S-NODE 只新增前置分支，其余 K8S 目标保持现有逻辑。 |
| `2026-06-03 00:00` | 创建需求与初版方案，目标收敛为提升主机场景容器事件关联准确性。 |
