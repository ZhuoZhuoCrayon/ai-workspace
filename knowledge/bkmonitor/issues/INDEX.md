...
| 文件                                                                                                     | 标签                                                        | 摘要                                                      | 更新日期       |
|------|------|------|---------|
| [APM 预计算适配共享数据源](./2026-05-14-apm-precalc-shared-multi-app/README.md) | `apm` `pre-calculate` `shared-datasource` `multi-app` `bmw` | 让单 BMW 预计算任务感知共享 data_id 下的多个应用，不破坏现有任务模型与持久化键 | 2026-05-14 |
...
| [APM 支持应用级别配置](./2026-03-04-apm-app-level-config/README.md)                                            | `apm` `service-config` `log-relation` `code-redefine` `code-remark` `app-level`     | 将 APM 服务关联配置与返回码备注从纯服务粒度扩展到应用级别，支持跨服务共享与全局配置                   | 2026-05-01 |
