...
| [优化告警详情主机日志关联准确性](./2026-04-15-alert-detail-host-collector-log-relation/README.md) | `alert` `log` `host-target` `collector` `log-relation` `accuracy` | 先修复日志类 HOST 告警的原始日志关联优先级，再接入主机关联采集项日志索引 | 2026-06-27 |
...
| [日志 UnifyQuery 环境变量白名单与 query_string 增强](./2026-03-05-log-uq-env-whitelist-and-query-string/README.md) | `log` `unify-query` `data-source` `query-string` `config` | 为日志 UnifyQuery 灰度白名单增加环境变量配置层，并对齐日志平台 query_string 处理逻辑 | 2026-06-10 |
