...
- A.2 三服务（`60009 / 60015 / 60016`）5/1 减量 ≈ `0.4 M`。
- 名单外 `60022`（cc=471）/ `60024`（cc=173）合计 ≈ `0.47 M`。

### b. 服务分类与拓扑（5/1 视角）

```text
[A.1] 已 ignore 且 4-29 实测生效  5 个
├── 10240 (R=120) → redis-data        CC=1   4-29 NEW=41 × 120 = 4.92 K（首发新活动）
├── 10234 (R=120)                     CC=1   4-29 NEW=8  × 120 = 0.96 K
├── 60025 (R=60)                      CC=1   静默
├── 10243 (R=120)                     CC=1   静默
└── 10253 (R=120)                     CC=1   静默

[A.2] 已 ignore 但 4-29 0 点未生效（16:30 后补屏蔽）  3 个
├── 60009 (R=60)  → hpyd / ams        CC: 471 → ignore   4-29 贡献 ≈ 5.5 K，5/1 预期归零
├── 60015 (R=60)  → hpyd / amspkg     CC: 304 → ignore   4-29 贡献 ≈ 10.0 K，5/1 预期归零
└── 60016 (R=60)  → hpyd / amspkg     CC: 356 → ignore   4-29 贡献 ≈ 16.8 K，5/1 预期归零
...
| --- | --- | ---: | ---: |
| 乐观 | A.2 ignore 生效 + 1 个 C 类触发 | `0.94 M` | `0.75 M` |
| 中性 | A.2 生效 + 2 个 C 类触发 | `1.57 M` | `1.26 M` |
| 悲观 | A.2 生效 + 3 个 C 类触发 + 名单外两个拉起 | `2.66 M` | `2.13 M` |
