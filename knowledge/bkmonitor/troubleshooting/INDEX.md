...
| 文件 | 标签 | 摘要 | 更新日期 |
|------|------|------|---------|
| [APM Trace 写入 ES 字段类型冲突](./apm-trace-es-field-mapping-conflict.md) | `apm` `trace` `elasticsearch` `mapping` `attribute-filter` `normaltypevalueconfig` | APM Trace 属性同时上报 upstream_cluster 和 upstream_cluster.name，触发 ES keyword 与 object mapping 冲突，可通过应用级 attribute_filter drop 配置止血 | 2026-07-02 |
