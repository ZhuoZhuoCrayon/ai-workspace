...
flowchart LR
    subgraph BG["容量负载采样（250 ms）"]
        CG["cgroup.Reader<br/>cgroup 读取 + 按配额归一化"] --> S["ResourceSampler<br/>CPU %（EWMA 平滑）/ Mem %"]
        S -->|原子发布| WL["WaterLevel<br/>(cpu, cpuSlow, cpuFast, mem)"]
        S --> ST["更新状态机<br/>Normal / Shedding / Open"]
    end
    subgraph REQ["限流决策"]
...
|-------------------------|-----------------|--------------------|------------------|---------------------|
| 开启限流                    | ❌               | ❌                  | ✅                | ✅                   |
| 并发                      | `-c 26`         | `-c 32`            | `-c 26`          | `-c 32`             |
| 持续时间                    | `12 min`        | `12 min`           | `12 min`         | `12 min`            |
| QPM 峰值                  | `3,504 req/min` | `3,519 req/min`    | `3,563 req/min`  | `3,810 req/min`     |
| 成功接收 Span 数 *[5]*       | `2,444,928`     | `2,156,042`        | `2,466,816`      | `2,441,728`         |
| 成功放行数（`200`） *[6]*      | `14,208`        | `13,377`           | `14,373`         | `14,288`            |
| 限流主动拒绝请求数（Span 数） *[7]* | `0（0）`          | `0（0）`             | `2,727（857,472）` | `11,397（4,352,256）` |
| OOM 请求超时丢失（Span 数）      | `0（0）`          | `4,960（2,476,278）` | `0（0）`           | `0（0）`              |
| Receiver 字节速率峰值         | `5.94 MiB/s`    | `5.93 MiB/s`       | `5.83 MiB/s`     | `5.91 MiB/s`        |
| Receiver 处理平均耗时峰值       | `1.122 s`       | `0.947 s`          | `1.184 s`        | `1.487 s`           |
| Receiver 处理耗时 P99 峰值    | `6.896 s`       | `8.572 s`          | `6.518 s`        | `9.363 s`           |
| CPU 使用率峰值（Limits）       | `119.45%`       | `111.13%`          | `117.54%`        | `120.93%`           |
| 内存使用率峰值（Limits）         | `81.96%`        | `73.55%`           | `78.14%`         | `79.43%`            |

* *[1] A：已稳定压满 CPU、内存，再增加并发、持续时间将触发 OOM。*
* *[2] B：关闭限流后连续触发 `4` 次 OOMKilled，持续加压将会更严重。*
* *[3] C：开启限流不损失吞吐（请求量、Span 成功处理数多于 A 组），耗时 P99 / 容器 & CPU 峰值均低于 A 组。*
* *[4] D：开启限流后过载稳定，成功放行数与 A 组持平，没有 OOM 或重启。*
...
| A：关闭限流，`-c 26` 未过载极值 | C：开启限流，`-c 26` 未过载极值 |
|----|----|
| ![A 组压测截图：关闭限流，-c 26 未过载极值](./images/loadtest-a-20260620-01.png) | ![C 组压测截图：开启限流，-c 26 未过载极值](./images/loadtest-c-20260625-01.png) |
| B：关闭限流，`-c 32` 过载饱和 | D：开启限流，`-c 32` 过载饱和 |
| ![B 组压测截图：关闭限流，-c 32 过载饱和](./images/loadtest-b-20260620-01.png) | ![D 组压测截图：开启限流，-c 32 过载饱和](./images/loadtest-d-20260625-01.png) |

长稳复核：D 组按 `-c 32 -d 1200s` 持续加压，三阶段合计 `1 h`，状态面板保持 `Normal`。

![D 组长稳压测截图：开启限流，-c 32 -d 1200s 持续加压 1 h](./images/loadtest-d-longrun-20260625-01.png)
