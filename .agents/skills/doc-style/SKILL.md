...
4. **写协议**：涉及接口、配置或数据结构时，先给可复原结构的示例，再按读者实现所需决定字段说明详略。

### c. 交付阶段

1. **收交付**：归并重复信息，重写标题层级和段落结构，补齐验收、风险、验证或下一步动作。
2. **读者回读**：从目标读者视角通读一遍，确认只依赖文档就能获得核心信息、层级关系与必要字段。
...
- 闭卷通读初稿，确认目标读者不需要追问“结构长什么样”“字段在哪一层”“下一步做什么”。
- 协议类文档必须能从示例和字段表复原层级关系，不能复原时先补结构示例或字段契约。
- 若文档依赖隐含上下文才能理解，补充最小必要上下文，而不是追加零散解释。

## 0x05 交付前检查

- 默认执行以下命令。
- 若命令失败，先修正文档再重新执行，直到通过。
- 文档检查默认执行三轨：`lint-md`、`markdownlint-cli2`、`scripts/check_doc_style.py`。
