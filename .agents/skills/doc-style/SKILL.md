...
| 分层 | 任何载体内容膨胀时，先按主题拆分。 | 不用长列表、长段落或大表格无限堆叠。 |
| 映射 | 处理“对象 -> 动作 / 入口 / 归属”等对应关系。 | 字段稳定且需要横向对齐时使用表格。<br />内容膨胀时先按主题分组。 |
| 草稿整理 | 先归并重复信息，再重写标题层级和段落结构。 | 不保留输入顺序导致的流水账。 |
| 评审交付 | 文档承担评审职责时，补齐验收、风险、验证或下一步动作。 | PR review 评论保留根因链路叙述，修复点、对比项等横向信息按需表格化。 |
...
2. `PLAN.md` 场景额外回读 [exemplars.md](references/exemplars.md) 中的评测基线，校准结构密度与载体选择方式。
3. 逐段压缩句子，只保留可判定、可复用、可验证的信息。
4. 检查章节职责是否混淆，例如不要把需求主干写进 `PLAN.md`，也不要把调研细节写进 `README.md`。
5. 检查是否存在超过 `5` 项的无序列表，若存在则先按主题分组，再选择短列表、表格或图文组合。
6. 检查每个表格是否真的需要横向比较，若不需要则改成短段落、短列表或小节分组。
7. 检查表格是否存在重复维度，若存在则上提为小节或分组。
8. 检查段落、列表或表格是否正在膨胀，若膨胀则先按主题分层。
9. 按信息关系选择载体，并消除口语化、自述式表达。
