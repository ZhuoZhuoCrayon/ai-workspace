...
## 0x04 Polish Gate 与审稿自检

完成初稿后，必须执行一次固定润色与审稿闭环。

【CRITICAL（必须执行，不可协商）】审稿自检必须执行，不能用自动检查替代。
...
Polish gate 通过后，必须输出 `5` 张主观审稿命中清单。

执行约束：

- **只列举命中点，不直接改文档**，由作者评估后再决定是否采纳。
- 每张清单逐条给出原文位置（小节号或原句片段）和建议替换 / 改写方向。
- 命中为空时写"无"，不要省略清单。
