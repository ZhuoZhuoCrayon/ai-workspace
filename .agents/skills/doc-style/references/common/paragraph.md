...
- 表格、图、协议示例和核心伪代码已经表达字段、映射、层级、流程或协作关系时，删除复述这些关系的段落和列表。
- 文字只补充结构无法直接表达的原因、边界、例外、默认值、兼容策略、异常语义和决策后果。
- 同一信息只保留一个主要载体，其他载体只交叉引用或补充不同维度。
- 不为删除一句短说明而创建复杂结构，选择能准确表达关系的最小载体。
...
| 层级 / 归属 / 类关系     | 树或类图            | 节点和边表达关系，文字只补边界       |
| 分支 / 算法 / 模块协作   | 核心伪代码           | 保留关键判断，不展开实现细节         |
| 协议 / 字段契约       | 主表 + 子结构小表     | 见 0x03 协议类文档         |

### b. 表格

- 维度值重复时，上提为小节或分组，组内只保留需要比较的列。
- 表格不超过 `2` 行时，优先使用短段落或短列表。

**表格内换行**

同一单元格包含多项内容时，使用 `<br />` 换行，每行以 `[a]`、`[b]` 等字母标识区分。

```markdown
| 规则类型 | 说明 |
| --- | --- |
| 服务规则 | [a] 同一错误码可定义多条服务规则<br />[b] 各规则的 `service_names` 不得重叠 |
```

### c. 图、表和代码块的结构附注

图、表和代码块的补充说明紧跟结构，以独立注脚承载：

```markdown
| 规则类型 | 数据特征 | 唯一语义 |
| --- | --- | --- |
| 全局规则 | `<is_global=true, service_names=[]>` | `kind + code` |
| 服务规则 | `<is_global=false, service_names=[svc-a, svc-b, ...]>` *[1]* | `kind + code + remark` *[2]* |

- *[1] 仅保存用户覆盖项，内置错误码默认备注不落库，由服务视角 `get` 后置补齐。*
- *[2] `service_names` 只表达这条规则的作用范围，不参与规则身份判定。*
```

代码和 Mermaid 源码不插入注脚标记，注脚直接写明对应的标识符或节点。
...
- *[1] URI 规则和 `time_align` 是独立查询条件，不进入主因果链。*
````
