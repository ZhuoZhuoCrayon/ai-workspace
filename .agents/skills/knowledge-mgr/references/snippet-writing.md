...
- 每个三级标题是一个自包含的片段单元：片段本身、参数含义、关键边界、易错点都写在该三级标题下；不要把对某个片段的说明拆成新的三级标题。
...
### c. 一个三级标题 = 一个自包含片段，说明不另起标题

Bad：把脚本和它的关键边界拆成两个三级标题，读者读完脚本还要翻到下一节才能拿到陷阱说明。

````md
### b. OTLP HTTP traces 探针
...
- busybox `date` 不支持 `%N`，timestamp 会变成 `1782652397N`。
- `kind: 1` 是 `SPAN_KIND_INTERNAL`。
````

Good：脚本和针对它的说明都收在同一个三级标题下，删掉额外的「关键点」标题。
...
- busybox `date` 不支持 `%N`，timestamp 会变成 `1782652397N`，receiver 仍 200 但下游静默丢弃。
- `kind: 1` 是 `SPAN_KIND_INTERNAL`，要测 server / client 行为时改 `2` / `3`。
````
