...
方案写作是一条推导链，不是把需求拆成代码任务。

1. 需求理解：阅读 issue，确认目标、非目标、边界和验收口径。
2. 现状建模：复原当前系统如何表达这个问题，确认已有对象、协议、流程和约束。
3. 结构判断：说明复杂度来自哪里，应由哪个稳定对象、协议或流程承载，最终在哪里收敛。
4. 架构设计：按 [0x03 架构设计](#0x03-架构设计) 规范展开对象关系、协议契约、核心流程和不变量，再按 [0x08 自检](#0x08-自检) 检查。
5. 开发方案：阅读「架构设计」，按 [0x04 开发方案](#0x04-开发方案) 规范把设计落到开发协作契约，再按 [0x08 自检](#0x08-自检) 检查。
6. 验收与验证：按 [0x05 验收与验证](#0x05-验收与验证) 将需求验收、结构风险和回归边界转成可执行门禁。
7. 其他章节：按「架构设计」「开发方案」「验收与验证」编写实施进展、参考和版本锚点。

**【CRITICAL（必须执行，不可协商）】** 禁止跳过现状建模直接写架构设计，禁止在未完成架构设计的情况下编写开发方案。

### c. 信息归属

写作前先确定信息的唯一载体。载体选择错误时，方案会变成会话补丁或任务清单，读者需要自己重新拼出设计。

| 载体 | 只承载 |
| --- | --- |
| 调研与约束 | 影响结构判断的事实、约束、选型结论和不做范围。 |
| 架构设计 | 结构判断、对象关系、协议契约、核心流程、边界和不变量。 |
| 架构图 | 对象关系、协议流、数据流、状态机或职责分层。 |
| 开发方案 | 责任主题、声明位置、调用关系、收敛位置、对象去留和实现边界。 |
| 验收与验证 | 测试门禁、必测断言、回归口径和风险反证。 |
| 实施进展 | 阶段性结论与验证结果。 |
| 版本锚点 | 里程碑、分支、PR 状态。 |
...
架构设计必须先复原当前系统如何表达问题。阅读 issue、相关知识对象、代码和必要参考资料，确认需求范围内的对象模型、术语、协议关系、核心流程和约束。

调研结论不是背景材料，而是结构判断的证据链。无法确认的事实降级为「待确认」，并写清需要确认的位置。
...
* 明确混乱来源：不停留表象，寻找核心矛盾，例如语义混用、边界泄漏、规则重复、职责错位或生命周期不清。
* 一句话说明收敛点：当前差异从哪里来，应由哪个对象或协议承载，最终在哪个稳定入口收敛，复杂度来源必须能解释为什么局部补丁不够。
* 注重回归：若说不清差异归属，就没有进入架构设计，需要回到第一步，继续阅读代码和相关知识对象，直到找到核心收敛点。

4）围绕核心收敛点补齐调研：
* 项目调研：当前项目是否已具备或正在具备核心收敛点所需的对象、协议或流程。
* 领域机制：当方案依赖某个技术机制、行业模式或运行约束时，先解释机制，再给设计结论。
* 外部参考：必要时查找通用设计模式、开源项目或最佳实践，避免凭空想象。
...
6）围绕核心架构图分主题明确核心协议：
* 禁止强行套用 「关键决策 / 不变量 / 边界」。
* 按真实设计拆分主题，进一步明确对象职责、协议字段、状态转移、调用关系、边界条件和不变量。
...
核心：开发方案承接架构设计，把结构判断翻译成开发协作契约。它回答「在哪声明」「谁来调用」「在哪收敛」，并说明如何复用现有能力、补齐缺口和守住边界。
...
* 已有能力：说明由哪个对象、协议、入口或约束保证，避免重复设计。
* 可复用能力：说明复用方式，以及新增步骤插入到既有结构的哪个位置。
* 新增缺口：说明缺什么、为什么现有能力不能覆盖、需要在哪补。
* 不做范围：说明本期明确不处理的行为、兼容路径或自动化动作。
...
6）处理兼容与迁移：如果涉及旧协议、旧数据、旧入口或灰度路径，写清兼容策略、迁移方式和废弃边界。
...
* 检查推导链是否完整：需求边界 → 现状建模 → 结构判断 → 架构设计 → 开发方案 → 验收与验证。
* 检查调研证据是否能追溯到 issue、知识对象、代码、配置、测试或外部资料；无法确认的内容必须标为「待确认」。
* 按 [0x02.c 信息归属](#c-信息归属) 做语义去重：同一判断跨图、伪代码、正文、表格或版本锚点重复时，只保留最贴近读者决策的一处。
* 检查架构设计是否说明系统如何重新组织，而不是只列新增功能、文件或任务。
* 检查开发方案是否逐项承接架构结论，并回答「在哪声明」「谁来调用」「在哪收敛」。
* 检查验收与验证是否集中承载测试门禁、必测断言和回归口径，没有散落在架构设计或开发方案中。
