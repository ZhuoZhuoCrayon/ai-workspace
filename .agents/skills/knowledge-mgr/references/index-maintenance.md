...
1. 读取变更文件 frontmatter。
2. 若对象是目录级知识对象，读取其 `README.md`。
3. 无需先读全文。
4. 若对应层级 `INDEX.md` 不存在，按本文件模板新建。
5. 更新对应条目的标签、摘要、日期。
6. 向上逐级重复 4-5，直到项目级或一级索引。
...
| 文章 | [articles/](./articles/INDEX.md) | N 篇 | ... |
...
- 数量统计按逻辑单元计数：issue / article 目录各计 `1`，snippet / troubleshooting 各文件计 `1`。
- 仅统计当前可见性路径内的数量，不跨 public / private 合并。
- 最近更新保留最近 `5` 条。
- 动作词使用：`新增`、`更新`、`归档`、`迁移`、`删除`。
